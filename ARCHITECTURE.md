...
- A migração adiciona o status sem alterar custos antigos. Uma reimportação pode associar transações legadas ainda sem origem pela identidade contábil; quando a planilha não informa o custo de uma aquisição que o parser antigo havia corrigido, a operação existente é preservada e marcada como pendente em vez de ser duplicada. Decisões persistidas não são reclassificadas por importações posteriores de históricos mais antigos.

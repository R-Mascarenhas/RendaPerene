...
- `yfinance` supplies B3 quotes, price history, and dividend/valuation data. Tickers are requested with the `.SA` suffix. When a live quote is missing, zero, or non-finite, the integration falls back to the latest positive finite daily close before applying valuation rules.

...
if legacy_sources:
    ...
    if st.sidebar.button("Importar carteiras antigas", use_container_width=True):
        ...
        for source in selected_legacy:
            try:
                result = app_paths.migrate_legacy_database(source, migration_destinations[source])
            except ValueError:
                st.sidebar.error(f"{source.name}: use um nome de arquivo local terminado em .db.")
                continue
            if result.migrated:

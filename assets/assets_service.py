        ...
        for _, row in df.iterrows():
            try:
                ...
                if transaction_type == "Compra":
                    success = AssetService.add_transaction(ticker, date, "Compra", quantity, price)
                    if success: processed_transactions += 1
                elif transaction_type == "Venda":
                    success = AssetService.add_transaction(ticker, date, "Venda", quantity, price)
                    if success: processed_transactions += 1
                elif transaction_type == "Desdobro_Credito":
                    success = AssetService.add_transaction(ticker, date, "Compra", quantity, 0.0)
                    if success: processed_transactions += 1

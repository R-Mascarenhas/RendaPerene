...
class DashboardView:
    ...
    def render(self):
        ...
        # Pull planned contribution from Session State (Calculated in Planning Tab)
        required_monthly_contribution = st.session_state.get('required_monthly_contribution_cache', 0.0)
    ...
    def _render_charts(self, df_positions):
        ...
        with chart_col1:
            df_chart_profit = df_positions.sort_values(by="profit_loss", ascending=True)
            fig_profits = px.bar(
                df_chart_profit, 
                x="ticker", 
                y="profit_loss",
                color="profit_loss",
                color_continuous_scale=["#FF4B4B", "#29B05C"], 
                title="Resultado Líquido (Lucro/Prejuízo) por Ativo",
                labels={"profit_loss": "Resultado (R$)", "ticker": "Ticker"}
            )
            fig_profits.update_traces(hovertemplate="<b>%{x}</b><br>Resultado: R$ %{y:,.2f}<extra></extra>")
            fig_profits.update_layout(yaxis_tickformat="R$ ,.2f")

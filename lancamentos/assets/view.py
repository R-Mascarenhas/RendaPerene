...
class AssetsView:
    ...
    def _render_single_asset_subtab(self, ticker, df_positions):
        ...
        with col_img:
            ...
            # The onerror fallback uses a dynamic vector SVG with the sector color and ticker text
            github_logo_url = f"https://raw.githubusercontent.com/thefintz/icones-b3/main/icones/{ticker}.png"
            svg_fallback = f"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='108' height='108'><square cx='40' cy='40' r='38' fill='{fallback_color}'/><text x='40' y='46' fill='white' font-size='18' font-family='sans-serif' font-weight='bold' text-anchor='middle'>{ticker[:4]}</text></svg>"

            st.markdown(
                f'<img src="{github_logo_url}" referrerpolicy="no-referrer" onerror="this.onerror=null; this.src=\'{svg_fallback}\';" style="width: 108px; height: 108px; border-radius: 10%; object-fit: contain; background-color: white;">',

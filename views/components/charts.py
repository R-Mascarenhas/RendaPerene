...
class DashboardCharts:
    ...
    def _render_top_charts(self, df_positions):
        ...
        with chart_col2:
            ...
            df_chart_evol = df_chart_evol.rename(
                columns={INVESTED_AMOUNT: "Investido", CURRENT_VALUE: "Atual"}
            )
    ...
    def _render_evolution_chart(self):
        ...
        if not df_evolution.empty:
            ...
            bar_labels = [
                f"R$ {val:,.0f}".replace(",", ".") if val > 0 else ""
                for val in df_evolution[MONTHLY_DIVIDEND]
            ]
            ...
            fig_multi.update_xaxes(
                title_text="Linha do Tempo (Mês/Ano)",
                type="category",
                tickangle=-45,
                tickmode="linear",
            )

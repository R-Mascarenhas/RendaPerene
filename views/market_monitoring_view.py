...
from core.strings import (
    ...
    DISPLAY_ROE,
    DISPLAY_TICKER,
    HELP_MARKET_MONITOR_SEARCH,
    MODEL_CLASSIC,
    MODEL_IPCA_SPREAD,
    MODEL_SELIC,
    MSG_ADJUST_DIVIDENDS_DESC,
    MSG_ADJUST_DIVIDENDS_EXPANDER,
    MSG_ASSET_ADD_ERROR,
    MSG_ASSET_ADDED_SUCCESS,
    MSG_ASSET_REMOVED_SUCCESS,
    MSG_CONFIRM_REMOVE,
    MSG_DIVIDEND_CORRECTION_ERROR,
    MSG_DIVIDEND_CORRECTION_SUCCESS,
    MSG_INVALID_ASSET_CORRECTION,
    MSG_INVALID_ASSET_SELECTION,
    MSG_MARKET_MONITOR_DESC,
    MSG_MARKET_MONITOR_TITLE,
    MSG_MONITORED_ASSETS_PANEL,
    MSG_NO_ASSETS_MONITOR,
    MSG_YF_FETCH_ERROR,
)
...
class MarketMonitoringView:
    ...
    def _render_monitoring(self):
        ...
        with col_add, st.form("form_add_market_asset", clear_on_submit=True):
            # Construct autocompleting ticker + name options for premium UX
            market_options = ["--- Selecione ---"] + [
                f"{ticker} - {name}" for ticker, name in catalog_entries
            ]
        ...
        with col_yield:
            ...
            default_index = (
                model_options.index(default_db_model) if default_db_model in model_options else 0
            )
            ...
            if model == MODEL_SELIC:
                ...
                ipca_val = target_context["reference_rate"]
                st.caption(
        ...
        with col_rem:
            ...
            if db_only_tracked:
                remove_ticker = st.selectbox(
                    "Remover empresa do monitor", ["--- Selecione ---"] + db_only_tracked
                )
                if remove_ticker != "--- Selecione ---" and st.button(
                    MSG_CONFIRM_REMOVE.format(ticker=remove_ticker)
                ):
        ...
        for yr in last_5_years:
            col_configs[f"Div {yr}"] = st.column_config.NumberColumn(
        ...
        st.dataframe(styled_display, width="stretch", hide_index=True, column_config=col_configs)
            ...
            with st.form("form_dividend_correction", clear_on_submit=True):
                ...
                with col_c1:
                    corr_ticker = st.selectbox(
                        "Selecione o Ativo para Corrigir",
                        options=["--- Selecione ---"] + tracked_tickers,
                    )
                with col_c2:
                    current_year = datetime.date.today().year
                    corr_year = st.selectbox(
                        "Selecione o Ano", options=[current_year - i for i in range(1, 6)]
                    )
                with col_c3:
                    corr_value = st.number_input(
                        "Valor Total Pago no Ano (R$)", min_value=0.01, value=2.00, step=0.05
                    )
                ...
                if submit_corr:
                    if corr_ticker == "--- Selecione ---":
                        ...
                        success = AssetService.save_dividend_correction(
                            corr_ticker, corr_year, corr_value
                        )
                        if success:
                            st.success(
                                MSG_DIVIDEND_CORRECTION_SUCCESS.format(
                                    ticker=corr_ticker,
                                    year=corr_year,
    ...
    def _on_bazin_model_change(self):
        ...
        st.session_state[SESSION_CEILING_MODEL_SELECTION] = st.session_state[
            WIDGET_CEILING_MODEL_SELECTOR
        ]
    ...
    def _on_bazin_yield_change(self):
        ...
        st.session_state[SESSION_BAZIN_TARGET_YIELD] = float(
            st.session_state[WIDGET_BAZIN_YIELD_INPUT]
        )
        self._save_market_params()
    ...
    def _on_bazin_spread_change(self):
        ...
        st.session_state[SESSION_BAZIN_TARGET_SPREAD] = float(
            st.session_state[WIDGET_BAZIN_SPREAD_INPUT]
        )
        self._save_market_params()
